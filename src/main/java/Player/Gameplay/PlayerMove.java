...
public class PlayerMove{
    ...
    public static void moveToHomeBase() {
        long runFor = Math.round(5 * (distanceToHomeBase(PlayerApplication.getCoordX(), PlayerApplication.getCoordY())) * 1000 );
        try{
    ...
    public static void moveToAnotherPlayer(int coordX, int coordY, double distance) {
        ...
        try{
            Thread.sleep(runFor);
            PlayerApplication.setCoordXAndCoordY(coordX, coordY);
        } catch(InterruptedException e) {e.printStackTrace();}
    }
}
